...
pub fn init_emu_with_heap(arch: Arch, 
    ...
    unsafe {
        ...
        let arena_ptr = (&mut memmap2::MmapMut::map_anon(size as usize).expect("failed to allocate memory")[..]).as_ptr() as *mut c_void;
...
pub fn uc_alloc(uc: &mut super::UnicornHandle<RefCell<Heap>>, mut size: u64) -> Result<u64, uc_error> {
    ...
    let curr_chunk = Chunk {offset: curr_offset, len: size as usize, freed: false};
